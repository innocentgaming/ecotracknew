...
const KEYS = {
  ...
  ADVISOR_REPORT: 'ecotrack_advisor',
};
...
export function loadSettings() {
  try {
    ...
    const parsed = data ? JSON.parse(data) : { geminiApiKey: '', theme: 'dark' };
    if (!parsed.geminiApiKey && typeof process !== 'undefined' && process.env && process.env.NEXT_PUBLIC_GEMINI_API_KEY) {
      parsed.geminiApiKey = process.env.NEXT_PUBLIC_GEMINI_API_KEY;
    }
    return parsed;
  } catch (e) {
    const defaultSettings = { geminiApiKey: '', theme: 'dark' };
    if (typeof process !== 'undefined' && process.env && process.env.NEXT_PUBLIC_GEMINI_API_KEY) {
      defaultSettings.geminiApiKey = process.env.NEXT_PUBLIC_GEMINI_API_KEY;
    }
...
export function saveAdvisorReport(report) {
  try {
    localStorage.setItem(KEYS.ADVISOR_REPORT, JSON.stringify(report));
  } catch (e) {}
}

export function loadAdvisorReport() {
  try {
    const data = localStorage.getItem(KEYS.ADVISOR_REPORT);
    return data ? JSON.parse(data) : null;
  } catch (e) { return null; }
}
